...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (chats.length === 0) nuevoChat();
    ...
    //coger token csrf y ponerlo en el formulario
      fetch('/csrf-token').then(response => response.text()).then(token => {

            //Se pone el csrf en el input del form
            document.getElementById('crsf').value = token;

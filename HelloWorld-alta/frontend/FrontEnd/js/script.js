...
btnValidar.disabled = chat.validado;

mostrarHistorial();

}
// ===========================
//   VALIDAR CASO CLÍNICO
// ===========================

function validarCasoClinico() {
  if (!window.chatActivo) return;

  const chats = cargarChats();
  const chat = chats.find(c => c.id === window.chatActivo);
  if (!chat || chat.mensajes.length === 0) return;

  if (chat.validado) {
    alert("Este caso clínico ya ha sido validado.");
    return;
  }
  ...
  chat.mensajes.push({
    rol: "ai",
    texto: "✅ Caso clínico validado correctamente. El plan diagnóstico y terapéutico es coherente con la información proporcionada."
  });
...
function eliminarChat(id) {
  ...
  if (window.chatActivo === id && chats.length) {
...
document.querySelector("#btn-validar").onclick = validarCasoClinico;
